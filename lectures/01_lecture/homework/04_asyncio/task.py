...
async def run_task_group(names: list[str]) -> dict[str, str | None]:
    ...
    try:
        async with asyncio.TaskGroup() as task_group:
            for name in names:
                tasks[name] = task_group.create_task(
    ...
    for name, task in tasks.items():
        ...
        results[name] = None if error is not None else task.result()

    if results and all(value is None for value in results.values()):
        return {}
    return results
...
async def fetch_with_timeout(url: str, delay: float, timeout: float) -> str:
    ...
    return await asyncio.wait_for(
        asyncio.sleep(delay, result=f"data:{url}"),
        timeout=timeout,
    )
